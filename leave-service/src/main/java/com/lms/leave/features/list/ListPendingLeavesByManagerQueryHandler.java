package com.lms.leave.features.list;

import com.lms.leave.common.mediator.IQueryHandler;
import com.lms.leave.common.result.Result;
import com.lms.leave.features.LeaveResponse;
import com.lms.leave.infrastructure.persistence.entity.LeaveStatus;
import com.lms.leave.infrastructure.persistence.repository.LeaveRequestRepository;
import org.springframework.stereotype.Component;

import java.util.List;

@Component
public class ListPendingLeavesByManagerQueryHandler
        implements IQueryHandler<ListPendingLeavesByManagerQuery, Result<List<LeaveResponse>>> {

    private final LeaveRequestRepository leaveRequestRepository;

    public ListPendingLeavesByManagerQueryHandler(LeaveRequestRepository leaveRequestRepository) {
        this.leaveRequestRepository = leaveRequestRepository;
    }

    @Override
    public Result<List<LeaveResponse>> handle(ListPendingLeavesByManagerQuery query) {
        List<LeaveResponse> leaves = leaveRequestRepository
                .findAllByManagerIdAndStatus(query.getManagerId(), LeaveStatus.PENDING)
                .stream()
                .map(LeaveResponse::from)
                .toList();
        return Result.success(leaves);
    }
}
